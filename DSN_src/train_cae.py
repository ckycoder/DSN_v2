...
dataset = {x : SLC_spe_xy(txt_file=txt_file[x], spe_dir='../data/spe_data_32/', sublook=32,
                                       spe_transform = data_transforms)
           for x in ['train', 'val']}
print('len(dataset[train])',len(dataset['train']))#2550 * 32 * 32
    ...
    for sample in dataloader['train']:
        ...
        if i % 10 == 0:
            ...
            if i % 100 == 0:
                ...
                if i % 1000 == 0 or loss.item()<0.00041:
                    torch.save(net.state_dict(), save_model_path + 'iter' + str(i+30000) + '.pth')
        i += 1
