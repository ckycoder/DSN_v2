...
save_path = '../data/spexy_data_16/'


net = network.SLC_spexy_CAE_16()
net.load_state_dict(torch.load('../model_16/slc_spexy_cae_3_iter215000.pth'))

dataset = SLC_spe_4D(txt_file=txt_file, spe_dir='../data/spe_data_16/', spe_transform=data_transforms)
...
# spe_3D = torch.Tensor(np.zeros([batch_size, 128, 32, 32]))
sublook = 16
spe_3D = np.zeros([batch_size, 256, sublook, sublook])
...
for sample in dataloader:
    ...
    if flag:
        for y in range(sublook):
            ...
            data_out = net.get_encoder_features(data_xy).cpu().data.reshape([data.shape[0], sublook, 256])
