    ...
    for cate in slc_list:
        ...
        for data in data_list:
            if not os.path.exists(spe_root + cate + '/' + data):
                ...
                spectrogram = np.log(1+np.abs(gen_spectrogram_2(slc_data, 48)))#(16,32,48)

                np.save(spe_root + cate + '/' + data, spectrogram)

def gen_spe_data(slc_root,spe_root):
    slc_list = os.listdir(slc_root)
    for cate in slc_list:
        if not os.path.exists(spe_root + cate):
            os.mkdir(spe_root + cate)
        ...
        for data in data_list:
            if not os.path.exists(spe_root + cate + '/' + data):
                slc_data = np.load(slc_root + cate + '/' + data)
                spectrogram = np.log(1 + np.abs(gen_spe(slc_data)))
                # print(spectrogram)
                np.save(spe_root + cate + '/' + data, spectrogram)
...
def gen_train_val(data_root):
    val_ratio = 0.3#related to 'slc_train(val)_num(1,2,3...22).txt'
    ...
    df_train.to_csv('../data/slc_train_3.txt', index=False)
    df_val.to_csv('../data/slc_val_3.txt', index=False)
...
if __name__ == '__main__':
    ...
    spe_root = '../data/spe_data_48/'#(16,32,48)
    gen_all_spec(slc_root, spe_root)#gen spe_data:(64*64)->(32*32*32*32)
    # gen_spe_data(slc_root, spe_root)

    # get_mean_std_img(spe_root)
    # get_range_spec(spe_root)
    #gen_train_val(slc_root)#gen train.txt and val.txt
