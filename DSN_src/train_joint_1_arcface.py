...
if __name__ == '__main__':
    ...
    batch_size = {'train': 256,
                  'val': 64}
    cate_num = 8
    save_model_path = '../model_multi'+str(datasetnum)+'/slc_joint_arc_' + str(datasetnum) + '_F_'
    ...
    net_joint = network.SLC_joint2_arc(cate_num,s=20,margin=0.5)
    ...
    f_acc = open('../model/arc_accuracy'+str(datasetnum)+'.txt','w')
    f_loss = open('../model/arc_loss'+str(datasetnum)+'.txt','w')
    for epoch in range(epoch_num):
        ...
        for data in dataloaders['train']:
            ...
            output,_ = net_joint(spe_data_32, spe_data_48,spe_data_16, img_data, labels)
        ...
        for j in range(len(dataloaders['val'])):
            ...
            _, val_output = net_joint(val_spe_32,val_spe_48,val_spe_16, val_img, val_label)
